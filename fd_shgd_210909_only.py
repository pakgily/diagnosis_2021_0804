...
''' 입력 정보 '''
data_filename = 'D://시스템 응용관련//00.업무관련//01.프로젝트//6속 HEV DCT//7.검증 시험//시험데이터//고장진단 정합성 검증//210909_저온_disengage.dat'
...
sfcon_odd = AnalySig[(AnalySig['rbm_TTCur'] == 2) & (AnalySig['dmm_GBTgtGearOddOld'] != 'gN') & (
            AnalySig['sgm_ShiftState1'] == 'S_ShfBackToNSlot')]
sfcon_even = AnalySig[(AnalySig['rbm_TTCur'] == 2) & (AnalySig['dmm_GBTgtGearEvenOld'] != 'gN') & (
            AnalySig['sgm_ShiftState2'] == 'S_ShfBackToNSlot')]
...
odd_ShfActFrc_interval = pd.cut(sfcon_odd['gam_ShfActFrc1'].abs(), [0, 300, 600, 900, 1200], right=True,
                                include_lowest=True)  # 1번인자 : 배열 / 2번인자 : 구간 / right : < A << / inclue_lowest = True 최소 값 포함 여부
odd_ActFrc_ShfState_totaltime = sfcon_odd[['dmm_GBTgtGearOddOld', 'gam_ShfActFrc1']].groupby(
    ['dmm_GBTgtGearOddOld', odd_ShfActFrc_interval]).count() * 0.01
...
even_ShfActFrc_interval = pd.cut(sfcon_even['gam_ShfActFrc2'].abs(), [0, 300, 600, 900, 1200], right=True,
                                 include_lowest=True)  # 1번인자 : 배열 / 2번인자 : 구간 / right : < A << / inclue_lowest = True 최소 값 포함 여부
even_ActFrc_ShfState_totaltime = sfcon_even[['dmm_GBTgtGearEvenOld', 'gam_ShfActFrc2']].groupby(
    ['dmm_GBTgtGearEvenOld', even_ShfActFrc_interval]).count() * 0.01
...
all_gear_ActFrc_ShfState_totaltime = pd.concat([odd_ActFrc_ShfState_totaltime, even_ActFrc_ShfState_totaltime],
                                               axis=0).sort_index(axis=0)
...
while i < len(all_data_search_start_index):
    ...
    while j < len(all_data_search_start_index[i]):
        ...
        if i == 0:
            ...
            AnalySig.iloc[all_data_search_start_index[i][j]:all_data_search_end_index[i][j] + 1]['gam_ShfActFrc1'].abs()
        ...
        if i == 1:
            ...
            AnalySig.iloc[all_data_search_start_index[i][j]:all_data_search_end_index[i][j] + 1]['gam_ShfActFrc2'].abs()
