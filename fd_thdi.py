...
even_Analy_All = pd.DataFrame(even_Analy_All,columns=['Turn On Time','Current Mean', 'Current Max'])
...
odd_DI_Diff_Interval = pd.cut(odd_Analy_All['Damage Index Diff'],[0, 0.05, 0.1, 0.2, 0.3], right=False, include_lowest=True,precision=2) # 1번인자 : 배열 / 2번인자 : 구간 / right : < A << / inclue_lowest = True 최소 값 포함 여부
odd_Current_Mean_Interval = pd.cut(odd_Analy_All['Current Mean'],[0,3,5,7,9,11], right=False, include_lowest=True,precision=2) # 1번인자 : 배열 / 2번인자 : 구간 / right : < A << / inclue_lowest = True 최소 값 포함 여부
odd_Current_Max_Interval = pd.cut(odd_Analy_All['Current Max'],[0,3,5,10,20,40], right=False, include_lowest=True,precision=2) # 1번인자 : 배열 / 2번인자 : 구간 / right : < A << / inclue_lowest = True 최소 값 포함 여부
