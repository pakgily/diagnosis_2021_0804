...
writer = pd.ExcelWriter('sample_test.xlsx', engine='xlsxwriter')
...
while i < len(Analy):
    Analy[i].to_excel(writer,sheet_name='Result', startrow=start_row, startcol=0)
    Analy_shape.append(Analy[i].shape[0]+3)
...
# Analy_1.to_excel(writer, sheet_name='Result', startrow=1, startcol=0)
# Analy_2.to_excel(writer, sheet_name='Result', startrow=Analy_1.shape[0]+5, startcol=0) #shape[0] : 행 갯수 / shape[1] 열 갯수
# Analy_3.to_excel(writer, sheet_name='Result', startrow=Analy_2.shape[0]+5, startcol=0)
