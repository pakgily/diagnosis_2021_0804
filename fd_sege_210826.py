...
def run(AnalySig):
    ...
    while l < len(All_data_search_start_index): #4개
        ...
        while k < 4: # 4개의 sample(40ms) 동안 값이 변하지 않을 경우 Active Counter저장하기 위한 반복
            if(l==0): #odd_L/H에 관해 수행
                if len(All_data_search_start_index[l]) != 0: # 데이터가 없을 경우를 위해서
            ...
            if (l==1):  # odd_L/H에 관해 수행
                if len(All_data_search_start_index[l]) != 0:
                    if AnalySig['sgm_CounterSolHighActive1'][All_data_search_start_index[l][i] + j + 1] == \
                            AnalySig['sgm_CounterSolHighActive1'][All_data_search_start_index[l][i] + j]:
                        j += 1
                        k += 1
                        if (All_data_search_start_index[l][i] + j) == count - 1:  # Search 종료 시점을 정의 Active Counter 값의 마지막에서 종료
                            break
                        if k == 4:
                            active_count.append(AnalySig['sgm_CounterSolHighActive1'][All_data_search_start_index[l][i] + j])  # 4개의 sample(40ms) 동안 값이 변하지 않을 경우 Active Counter저장
                            if i == len(All_data_search_start_index[l]) - 1:  # Search 종료 시점을 정의 Active Counter 값의 마지막에서 종료
                                break
                            i += 1
                            j = 0
            ...
            if (l == 3):
                if len(All_data_search_start_index[l]) != 0:
                    if AnalySig['sgm_CounterSolHighActive2'][All_data_search_start_index[l][i] + j + 1] == AnalySig['sgm_CounterSolHighActive2'][All_data_search_start_index[l][i] + j]:
                        j += 1
                        k += 1
                        if (All_data_search_start_index[l][i] + j) == count - 1:
                            break
                        if k == 4:
                            active_count.append(AnalySig['sgm_CounterSolHighActive2'][All_data_search_start_index[l][i] + j])
                            if i == len(All_data_search_start_index[l]) - 1:
                                break
                            i += 1
                            j = 0
    ...
    odd_L_count_analy.sort_index(axis=0, ascending=True,inplace=True)
    ...
    odd_H_count_analy.sort_index(axis=0, ascending=True, inplace=True)
    ...
    even_L_count_analy.sort_index(axis=0, ascending=True, inplace=True)
    ...
    even_H_count_analy.sort_index(axis=0, ascending=True, inplace=True)
